...
let resultPage = 1;
let url = `${host}/games?steamspy_tags=${queries}`;


const getTags = async () => {
    try {
        const url = `${host}/steamspy-tags?limit=9999`;
...
function createGamesResult(game) {
    ...
    x.classList.add('game-item', 'd-flex', 'align-items-center');
...
function createTags(tag) {
    ...
    x.innerHTML = `<a href="tags.html?${tag.name}">${tag.name}</a>`
...
const renderGamesResults = async (games)=> {
    try {
        const genresGames = document.getElementById("tagsList");
...
const renderTags = async (tags)=> {
    try {
        const tagsList = document.getElementsByClassName("sidebar");
           
        tagsList[0].innerHTML = ""; 
        
        tags.forEach((tag) => {         
            tagsList[0].appendChild(createTags(tag));
        })
...
nextResults.addEventListener("click", async()=>{
    ...
    url = `${host}/games?steamspy_tags=${queries}`;
